...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function() external payable {
        ...
        // only accept KLAY via fallback from the WKLAY contract
    }
    ...
    function addLiquidityKLAY(
        ...
        address to,
        uint deadline
    ) external payable ensure(deadline) returns (uint amountToken, uint amountKLAY, uint liquidity) {
    ...
    function removeLiquidityKLAY(
        ...
        address to,
        uint deadline
    ) public ensure(deadline) returns (uint amountToken, uint amountKLAY) {
    ...
    returns (uint[] memory amounts)
    {
        ...
        assert(IWKLAY(WKLAY).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        if (msg.value > amounts[0]) TransferHelper.safeTransferKLAY(msg.sender, msg.value - amounts[0]);
        // refund dust eth, if any
    }
